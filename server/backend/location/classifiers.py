...
class Classifier:
    def __init__(self,clf, params, labels=None, label_names =None, scaler=None):
        self.labels = labels
    ...
    def predict_score(self, X_train, y_train, X_test, y_test):
        ...
        print classification_report(y_train, preds_train ,target_names=self.label_names, labels=self.labels )
        
        print "- Test Results -"
        preds_test = self.clf.predict(X_test)
        print classification_report(y_test, preds_test,target_names=self.label_names, labels=self.labels )

        #score
        train_scores = precision_recall_fscore_support(y_train, preds_train, labels=self.labels )     
        test_scores = precision_recall_fscore_support(y_test, preds_test, labels=self.labels )
        ...
        score = {
            "labels": self.label_names if self.label_names != None else range(0,len(train_scores[0])),
...
def initialize_classifiers():
    ...
    lvl_1_classifiers.append(Classifier(rfc, parameters,labels=[0],label_names=["Entity Word"]) )

    # Classifier 1:  LogisticRegression 0.39 
    from sklearn.linear_model import LogisticRegression
    parameters =  {
        "C":[0.08]  #[ 1.0, 0.5, 0.1, 0.08 ,0.05]
        }
    lrc = LogisticRegression(penalty="l1", random_state=238, n_jobs=4, )
    lvl_1_classifiers.append(Classifier(lrc, parameters,scaler="min_max",labels=[0],label_names=["Entity Word"]) )
    ...
    # parameters = {
    #     "learning_rate":[0.5], #[1,0.5,0.1,0.05], 
    ...
    parameters = {
        "C":[100] #[100.0,20.0,10.0,1,0.1,0.01]
    }
    svc_clf = SVC(random_state = 452)
    lvl_1_classifiers.append(Classifier(svc_clf, parameters,scaler="min_max",labels=[0],label_names=["Entity Word"] ) )
    ...
    lvl2 = Classifier(clf, parameters, labels=[0],label_names=["Entity Word"])
    ...
    lvl3 = Classifier(rfc3, parameters, labels=None)


    return lvl_1_classifiers, lvl2, lvl3